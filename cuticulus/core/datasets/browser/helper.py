...
from cuticulus.core.datasets.browser.node import Node


class Helper():
    def sort_dict(self, original_dict):
        sorted_dict = {'subfamily': None, 'genus': None,
                       'subgenus': None, 'species': None, 'subspecies': None}
    ...
    def update_data(self, data_helper):
        index = 1
        for i in range(3000):
            print("Updating data... (" + str(index) +
                  "/" + str(3000) + ")", end='\r')
            dict = data_helper.get_binomial(index)
            if dict == {}:
                consc_nulls += 1
            else:
                consc_nulls = 0
    ...
    def _add_node_rec(self, node, dict, dict_ind, img_id):
        if dict_ind < len(list(dict.keys())):
            ...
            if node is None:
                ...
                node.data = self._add_node_rec(
                    node.data, dict, dict_ind+1, img_id)
                node.data.set_type()
            else:
                prev_node = None
                root_node = node
