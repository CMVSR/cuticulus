...
class SpeciesPane(Pane):
    ...
    _species_taxon = {'subgenus': None, 'Genus': None,
                      'Sub-Family': None, 'Species': None, 'Sub-species': None}
    ...
    def _append_texts(self, h_key_str, h_val_str, delimiter, index, h_key_list, h_val_list):
        ...
        """
        if index > 0:
            self._append_texts(h_key_str, h_val_str, delimiter,
                               index-1, h_key_list, h_val_list)
        self._append_text(h_key_str, h_val_str, delimiter,
    ...
    def _append_text(self, h_key_str, h_val_str, delimiter, index, h_key_list=[], h_val_list=[]):
        ...
        h_key_list.append(self._p_font.render(
            h_key_split[index], True, PANE_BD_KEY_CLR))
        h_val_list.append(self._p_font.render(
            h_val_split[index], True, DFLT_FNT_CLR))
    ...
    def _render_text(self, h_key_list, h_val_list, index):
        ...
        self.surface.blit(h_key_list[index], (self._content_attrib['position']
                          [0], self._content_attrib['position'][1] + (index * 16)))
        self.surface.blit(h_val_list[index], ((
            self._content_attrib['position'][0] + 120), self._content_attrib['position'][1] + (index * 16)))
    ...
    def _render_texts(self, h_key_str, h_val_str):
        ...
        if len(h_key_split) != len(h_val_split):
            raise KeyError("The number of taxon keys (" + len(h_key_split) +
                           ") does not match the number of taxon values (" + len(h_val_split) + ")")
