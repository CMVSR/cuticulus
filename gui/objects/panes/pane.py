...
class Pane():
    ...
    def show(self):
        ...
        if self.title is not None:
            ...
            pygame.draw.rect(
                self.surface, const.PANE_HEADER_COLR, header_rect
            )
            head_text_object = self.h_font.render(self.title, True, g_const.DFLT_FNT_CLR)
            self.surface.bilt(head_text_object, (self.position[0], (self.position[1] + int(self.size.get("height")/3))))
            content_size = (self.pane_size.get("width"), (self.pane_size.get("height") - g_const.PANE_HEADER_HEIGHT))
            content_position = ((self.position[0] + g_const.PANE_HEADER_HEIGHT), self.position[1])
        else:
        ...
        pygame.draw.rect(self.surface, g_const.PANE_COLR, content_rect)
