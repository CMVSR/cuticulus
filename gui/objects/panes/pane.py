...
class Pane():
    ...
    def show(self):
        ...
        if self.title is not None:
            header_rect = pygame.Rect(self.position[0], self.position[1], int(
                self.pane_size.get("width")), 50)
            pygame.draw.rect(
                self.surface, g_const.PANE_HEADER_COLR, header_rect
            )
            head_text_object = self.h_font.render(
                self.title, True, g_const.DFLT_FNT_CLR)
            self.surface.blit(head_text_object,
                              (self.position[0], (self.position[1])))
            content_size = (self.pane_size.get(
                "width"), (self.pane_size.get("height") - g_const.PANE_HEADER_HEIGHT))
            content_position = (
                self.position[0], (self.position[1] + g_const.PANE_HEADER_HEIGHT))
        else:
            content_size = self.pane_size
            content_position = self.position
        content_rect = pygame.Rect(
            (content_position[0], content_position[1]), content_size)
